  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success && result.user) {
        ...
        let debugMsg = "An unexpected error occurred.";
        if (result.error) {
          debugMsg = `Login failed: ${result.error}`;
        } else if (!result.user) {
          debugMsg = "Login failed: No user returned from backend.";
        }
        setError(debugMsg);
      }
    } catch (err: any) {
      setError(`Login exception: ${err?.message || err}`);
    } finally {
      setIsPending(false);
    }
  }
