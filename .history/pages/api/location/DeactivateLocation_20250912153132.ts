...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
      ...
      if (!id && req.body.detail) {
        ...
        try {
          ...
          if (match && (match.locationID || match.LocationID || match.id || match.Id || match.ID)) {
            const resolved = match.locationID || match.LocationID || match.id || match.Id || match.ID;
            console.log('[DeactivateLocation] Matched location from list:', { matchKey, matchId, resolved, match });
            id = resolved;
          }
        } catch (e) {
  ...
  try {
    console.log('[DeactivateLocation] Deleting id=', id);
    const backendRes = await fetch(`http://svdcbas02:8212/api/location/DeactivateLocation/${id}`, {
      method: 'DELETE',
      headers: { ...(req.headers.cookie ? { Cookie: req.headers.cookie } : {}) },
    });
    const backendText = await backendRes.text();
    console.log('[DeactivateLocation] Backend delete status=', backendRes.status);
    console.log('[DeactivateLocation] Backend delete body:', backendText);
    // Safeguard: If response starts with '<!DOCTYPE', treat as HTML error
    if (backendText.trim().startsWith('<!DOCTYPE')) {
      return res.status(backendRes.status).json({
        message: `Backend returned HTML error page. Endpoint may not exist or is unreachable.`,
        html: backendText
      });
    }
    try {
