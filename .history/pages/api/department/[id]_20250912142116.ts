...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Use numericId for backend contract
  const numericId = typeof id === 'string' && /^\d+$/.test(id) ? Number(id) : id
  try {
    if (req.method === 'PUT') {
      console.log('[api/department/[id]] PUT id=', numericId, 'body=', JSON.stringify(req.body))
      const backendRes = await fetch(`http://svdcbas02:8212/api/Department/${numericId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', ...(req.headers.cookie ? { Cookie: req.headers.cookie } : {}) },
        body: JSON.stringify({ ...req.body, id: numericId }),
      })
    ...
    if (req.method === 'GET') {
      console.log('[api/department/[id]] GET id=', id)
      const backendRes = await fetch(`http://svdcbas02:8212/api/Department/${id}`, {
        method: 'GET',
        headers: { ...(req.headers.cookie ? { Cookie: req.headers.cookie } : {}) },
      })
      const backendText = await backendRes.text()
      console.log('[api/department/[id]] backend GET status=', backendRes.status, 'response=', backendText)
      forwardSetCookieFromResponse(backendRes, res)
      try { return res.status(backendRes.status).json(JSON.parse(backendText)) } catch { return res.status(backendRes.status).send(backendText) }
    }
