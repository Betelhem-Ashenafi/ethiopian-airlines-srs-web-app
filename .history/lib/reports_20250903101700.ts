...
export async function fetchReports(): Promise<Report[]> {
	...
	departmentName: r.DepartmentName ?? r.department?.DepartmentName ?? r.department?.departmentName ?? "",
	aiSeverity: r.SeverityName ?? r.severity?.SeverityName ?? r.severity?.severityName ?? "",
	severityName: r.SeverityName ?? r.severity?.SeverityName ?? r.severity?.severityName ?? "",
	statusName: r.StatusName ?? r.status?.StatusName ?? r.status?.statusName ?? "",
