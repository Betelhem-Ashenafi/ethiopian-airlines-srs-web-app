...
export async function fetchUserProfile(): Promise<Omit<User, "password"> | null> {
  try {
    const response = await fetch("http://svdcbas02:8212/api/auth/my-profile", {
      credentials: "include",
    });
...
export async function login(
  ...
  try {
    const response = await fetch("http://svdcbas02:8212/api/auth/login", {
      method: "POST",
      headers: {
...
export async function logout(): Promise<void> {
  try {
    await fetch("http://svdcbas02:8212/api/auth/logout", {
      method: "POST",
      credentials: "include", // 🔑 Sends cookies/session
    });
  } catch (err) {
