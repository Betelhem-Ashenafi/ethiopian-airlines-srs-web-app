...
export async function updateDepartment(payload: { DepartmentID?: number; DepartmentName: string; Description?: string; DepartmentEmail?: string; ContactNumber?: string; IsActive?: boolean }) {
  ...
  const tryPut = async (id: number) => {
    // Use backend contract: id, departmentName, description, departmentEmail, contactNumber, isActive
  ...
  const resolveByNameAndPut = async () => {
    try {
      ...
      if (listRes.ok) {
        ...
        if (match) {
          ...
          if (id) {
            try {
              return await tryPut(id)
            } catch (e) {
              throw new Error(`Update after resolve failed: ${String(e)}`)
            }
          }
        }
      }
  ...
  if (payload.DepartmentID && Number.isFinite(payload.DepartmentID)) {
    try {
      ...
      if (e?.status === 404) {
        const resolved = await resolveByNameAndPut();
        if (resolved) return resolved;
      }
      throw new Error(`Update failed: ${e?.status ?? ''} ${e?.text ?? String(e)}`)
    }
  }

  // Always try to resolve by name before creating
  const resolved = await resolveByNameAndPut();
  if (resolved) return resolved;
  ...
  if (!createRes.ok) {
    // If the backend reports a conflict (name already exists), always resolve by name and retry PUT
          ...
          if (match) {
            ...
            if (id) {
              try {
                ...
                // If PUT fails, throw error with details
                throw new Error(`Update after 409 failed: ${String(e)}`)
              }
            }
          }
        }
