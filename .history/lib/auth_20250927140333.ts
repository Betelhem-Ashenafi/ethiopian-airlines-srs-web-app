...
export async function login(
  ...
  if (process.env.NEXT_PUBLIC_USE_MOCK_DATA === 'true') {
    const { usersMock } = await import("@/mock/usersMock");
    const user = usersMock.find(u => u.employeeID === employeeId && u.password === password);
    if (user) {
...
export async function fetchUsers(): Promise<{ success: boolean; users?: any[]; error?: string }> {
  // Use mock data if NEXT_PUBLIC_USE_MOCK_DATA is set
  if (process.env.NEXT_PUBLIC_USE_MOCK_DATA === 'true') {
    const { usersMock } = await import("@/mock/usersMock");
    return { success: true, users: usersMock };
  }
  try {
