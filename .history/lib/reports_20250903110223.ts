...
export async function fetchReports(): Promise<Report[]> {
	...
	return backendReports.map((r: any) => {
		...
		const statusName = r.StatusName ?? r.Status?.StatusName ?? r.status?.StatusName ?? r.status?.statusName ?? "";
		const severityName = r.SeverityName ?? r.Severity?.SeverityName ?? r.severity?.SeverityName ?? r.severity?.severityName ?? "";
		const departmentName = r.DepartmentName ?? r.Department?.DepartmentName ?? r.department?.DepartmentName ?? r.department?.departmentName ?? "";
		const locationName = r.LocationName ?? r.Location?.LocationName ?? r.location?.LocationName ?? r.location?.locationName ?? "";
