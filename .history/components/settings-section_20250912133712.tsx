...
export default function SettingsSection() {
  ...
  const toggleDepartmentActive = async (deptId: string) => {
    ...
    try {
      const updated = await updateDepartment(serverId, payload)
      // update local entry with any normalized fields from server
      setDepartments(prev => prev.map(d => d.id === deptId ? ({
        ...d,
        isActive: updated.isActive,
        rawId: updated.rawId ?? d.rawId,
        id: updated.id ?? d.id,
      }) : d))
    } catch (e: any) {
      const msg = e?.message ?? String(e)
      // If server says not found, remove local item to stay in sync
      if (msg.includes('404') || msg.toLowerCase().includes('not found')) {
        setDepartments(prev => prev.filter(d => d.id !== deptId))
        alert('Department not found on server; removed from list.')
        return
      }
      // revert optimistic
      setDepartments(prev => prev.map(d => d.id === deptId ? { ...d, isActive: dept.isActive } : d))
      alert('Failed to update active: ' + msg)
    }
  }
