...
export function AuthProvider({ children }: {children :ReactNode }) {
  ...
  // Normalize user object from backend/localStorage to the app's expected shape
  function normalizeUser(u: any): Omit<User, "password"> {
    ...
    const rawRole = (u.role || u.Role || u.roleName || '').toString()
    const lower = rawRole.toLowerCase()
    const mapped = displayRoleMap[rawRole] || displayRoleMap[lower] || (rawRole ? (rawRole[0].toUpperCase() + rawRole.slice(1)) : 'Employee')

    const normalized: Omit<User, "password"> = {
      id: String(u.id || u.userId || u.UserID || u.id || ''),
      employeeID: String(u.employeeID || u.EmployeeID || u.employeeId || u.EmployeeID || u.id || ''),
      fullName: String(u.fullName || u.full_name || u.name || u.fullName || ''),
      email: String(u.email || u.Email || ''),
      role: mapped as "System Admin" | "Department Admin" | "Employee",
      status: (u.status || (u.isActive === false ? 'Inactive' : 'Active')) as "Active" | "Inactive",
      department: u.department || u.departmentName || u.departmentId || undefined,
    }
    return normalized
  }
