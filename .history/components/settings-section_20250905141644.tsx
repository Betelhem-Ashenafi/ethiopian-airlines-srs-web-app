...
export default function SettingsSection() {
  ...
  description: (obj as any)?.description ?? obj.Description ?? payload.Description ?? (payload as any)?.description,
  departmentEmail: (obj as any)?.departmentEmail ?? obj.DepartmentEmail ?? payload.DepartmentEmail ?? (payload as any)?.departmentEmail,
  contactNumber: (obj as any)?.contactNumber ?? obj.ContactNumber ?? payload.ContactNumber ?? (payload as any)?.contactNumber,
      }
      ...
      if (!res.ok) {
        ...
        if (res.status === 404) {
          ...
          try {
            ...
            if (getRes.ok) {
              // server knows about it; fetch server object and send full required fields when deactivating
  ...
  const res = await fetch(`/api/department/${serverId}`, {
      ...
      setDepartments(prev => prev.map(d => d.id === deptId ? ({
        ...d,
        isActive: (payload as any)?.IsActive ?? (payload as any)?.isActive ?? merged.IsActive ?? merged.isActive ?? d.isActive,
      ...
      if (!res.ok) {
        ...
        if (res.status === 404) {
          ...
          setDepartments(prev => prev.map(d => d.id === editDeptId ? ({
            ...
            name: payloadC?.DepartmentName ?? payloadC?.departmentName ?? merged.DepartmentName ?? d.name,
            description: payloadC?.Description ?? payloadC?.description ?? merged.Description ?? d.description,
            departmentEmail: payloadC?.DepartmentEmail ?? payloadC?.departmentEmail ?? merged.DepartmentEmail ?? d.departmentEmail,
            contactNumber: payloadC?.ContactNumber ?? payloadC?.contactNumber ?? merged.ContactNumber ?? d.contactNumber,
            isActive: payloadC?.IsActive ?? payloadC?.isActive ?? merged.IsActive ?? d.isActive,
          }) : d))
          closeEditDept()
          return
        }
