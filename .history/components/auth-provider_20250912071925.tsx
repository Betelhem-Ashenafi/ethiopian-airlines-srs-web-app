...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    async function restoreUser() {
      ...
      try {
        ...
        if (profile) {
          ...
          // ensure email (prefer backend, fallback to cached)
          merged.email = profile.email || cached?.email || ""

          // ensure fullName + employeeID
          const normalized = {
            ...merged,
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      // ✅ no redirect for logged-in users → refresh keeps them on same page
    }
  ...
  const handleLogin = (userData: Omit<User, "password">) => {
    ...
    userData = {
      ...userData,
      email: (userData as any).email || "", // ensure email is always set
    }
    setUser(userData)
