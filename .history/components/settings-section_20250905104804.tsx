...
export default function SettingsSection() {
  ...
  const [departments, setDepartments] = useState<{ id: string; name: string; description?: string; departmentEmail?: string; contactNumber?: string; isActive?: boolean; createdAt?: string; updatedAt?: string }[]>([])
  ...
  const handleUpdateDepartment = async (deptId: string, updated: { name?: string; description?: string; departmentEmail?: string; contactNumber?: string; isActive?: boolean }) => {
    try {
      // Merge with existing department object to send a complete DTO.
      const existing = departments.find(d => d.id === deptId)
      const merged = {
        departmentID: isNaN(Number(deptId)) ? undefined : Number(deptId),
        departmentName: updated.name ?? existing?.name ?? '',
        description: updated.description ?? existing?.description ?? '',
        departmentEmail: updated.departmentEmail ?? existing?.departmentEmail ?? '',
        contactNumber: updated.contactNumber ?? existing?.contactNumber ?? '',
        isActive: typeof updated.isActive === 'boolean' ? updated.isActive : (existing?.isActive ?? false),
      }
      ...
      const obj = updatedResp?.data ?? updatedResp
      // Update local state with backend response when available, otherwise use merged
      setDepartments(prev => prev.map(d => d.id === deptId ? {
        ...d,
        name: obj?.departmentName ?? merged.departmentName ?? d.name,
        description: obj?.description ?? merged.description ?? d.description,
        departmentEmail: obj?.departmentEmail ?? merged.departmentEmail ?? d.departmentEmail,
        contactNumber: obj?.contactNumber ?? merged.contactNumber ?? d.contactNumber,
        isActive: obj?.isActive ?? merged.isActive ?? d.isActive,
      } : d))
    } catch (e) {
      // fallback local update
      setDepartments(prev => prev.map(d => d.id === deptId ? { ...d, name: updated.name ?? d.name, description: updated.description ?? d.description, departmentEmail: updated.departmentEmail ?? d.departmentEmail, contactNumber: updated.contactNumber ?? d.contactNumber, isActive: updated.isActive ?? d.isActive } : d))
    }
  }
                    ...
                    <div className="mb-4">
                      <div className="flex flex-wrap items-center gap-2">
                        <Input
                          ...
                          className="min-w-0 w-44 h-8 text-sm px-2"
                        />
                        <Input
                          value={newDepartmentDescription}
                          onChange={(e) => setNewDepartmentDescription(e.target.value)}
                          placeholder="Description"
                          className="min-w-0 flex-1 h-8 text-sm px-2"
                        />
                        <Input
                          value={newDepartmentEmail}
                          onChange={(e) => setNewDepartmentEmail(e.target.value)}
                          placeholder="Contact email"
                          className="min-w-0 w-56 h-8 text-sm px-2"
                        />
                        <Input
                          value={newDepartmentContact}
                          onChange={(e) => setNewDepartmentContact(e.target.value)}
                          placeholder="Contact number"
                          className="min-w-0 w-40 h-8 text-sm px-2"
                        />
                        <Button onClick={handleAddDepartment} className="bg-et-green text-white h-8">Add</Button>
                      </div>
                    </div>
                    <div className="overflow-x-auto">
                      {/* use shared Table components to ensure consistent layout */}
                      <Table className="w-full text-sm table-fixed">
                        <TableHeader>
                          <tr className="text-left">
                            <TableHead className="px-1 py-1 text-sm whitespace-nowrap">Name</TableHead>
                            <TableHead className="px-1 py-1 text-sm whitespace-nowrap">Description</TableHead>
                            <TableHead className="px-1 py-1 text-sm whitespace-nowrap">Email</TableHead>
                            <TableHead className="px-1 py-1 text-sm whitespace-nowrap">Contact</TableHead>
                            <TableHead className="px-1 py-1 text-sm whitespace-nowrap">Active</TableHead>
                            <TableHead className="px-1 py-1 text-sm whitespace-nowrap">Actions</TableHead>
                          </tr>
                        </TableHeader>
                        <TableBody>
                          {departments.map((dept) => (
                            <TableRow key={dept.id} className="odd:bg-white even:bg-gray-50">
                              <TableCell className="px-1 py-1 align-top w-40 whitespace-nowrap">
                                <Input className="min-w-0 w-auto h-8 text-sm px-2" defaultValue={dept.name} onBlur={(e) => handleUpdateDepartment(dept.id, { name: e.target.value })} />
                              </TableCell>
                              <TableCell className="px-1 py-1 align-top whitespace-nowrap">
                                <Input className="min-w-0 w-auto h-8 text-sm px-2" defaultValue={dept.description} onBlur={(e) => handleUpdateDepartment(dept.id, { description: e.target.value })} />
                              </TableCell>
                              <TableCell className="px-1 py-1 align-top w-56 whitespace-nowrap">
                                <Input className="min-w-0 w-auto h-8 text-sm px-2" defaultValue={dept.departmentEmail} onBlur={(e) => handleUpdateDepartment(dept.id, { departmentEmail: e.target.value })} />
                              </TableCell>
                              <TableCell className="px-1 py-1 align-top w-36 whitespace-nowrap">
                                <Input className="min-w-0 w-auto h-8 text-sm px-2" defaultValue={dept.contactNumber} onBlur={(e) => handleUpdateDepartment(dept.id, { contactNumber: e.target.value })} />
                              </TableCell>
                              <TableCell className="px-1 py-1 align-top text-center">
                                <input className="h-4 w-4" type="checkbox" defaultChecked={Boolean(dept.isActive)} onChange={(e) => handleUpdateDepartment(dept.id, { isActive: e.target.checked })} />
                              </TableCell>
                              <TableCell className="px-1 py-1 align-top text-right">
