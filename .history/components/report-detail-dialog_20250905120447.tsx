...
// Helper to fetch department details by id
async function fetchDepartmentDetail(id: string) {
  ...
  return {
    ...
    name: data.departmentName ?? data.name ?? '',
...
export default function ReportDetailDialog({ report, isOpen, onClose }: ReportDetailDialogProps) {
  ...
  const [departmentDetail, setDepartmentDetail] = useState<DeptFields | null>(null)
  const [editingDepartment, setEditingDepartment] = useState(false)
  const emptyDeptFields: DeptFields = { id: '', name: '', description: '', departmentEmail: '', contactNumber: '', isActive: false };
  const [editDeptFields, setEditDeptFields] = useState<DeptFields>(emptyDeptFields);
  ...
  const handleSave = async () => {
    try {
      ...
      const json = await res.json()
      // Optionally refetch details after save
  ...
  // Fetch department details when dialog opens or department changes
  useEffect(() => {
    let mounted = true
    async function loadDept() {
      if (!isOpen) return
      // Try to get department id from departmentDropdown using name
      let deptId = null;
      const deptName = report.departmentName ?? report.aiDepartment ?? "";
      if (deptName && departmentDropdown && departmentDropdown.length) {
        const found = departmentDropdown.find(d => d.name === deptName);
        if (found) deptId = found.id;
      }
      if (!deptId) return setDepartmentDetail(null)
      try {
        const detail = await fetchDepartmentDetail(deptId)
        if (!mounted) return
  ...
  }, [isOpen, report?.departmentName, report?.aiDepartment, departmentDropdown])
  useEffect(() => {
    let mounted = true
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4">
                ...
                {departmentDetail && (
                  <div className="mb-2 p-2 border rounded bg-gray-50">
                    <div className="flex justify-between items-center">
                      <span className="font-semibold text-et-green">Department Details</span>
                      <Button size="sm" variant="outline" onClick={() => {
                        ...
                        setEditDeptFields({
                          ...
                          departmentEmail: departmentDetail.departmentEmail || '',
                          contactNumber: departmentDetail.contactNumber || '',
                          isActive: !!departmentDetail.isActive
                        })
                        setDeptError(null)
                      }}>Edit</Button>
                    </div>
                    {!editingDepartment ? (
                      <div className="grid grid-cols-2 gap-2 mt-2">
                        <div><span className="font-medium">Name:</span> {departmentDetail.name}</div>
                        <div><span className="font-medium">Description:</span> {departmentDetail.description}</div>
                        <div><span className="font-medium">Email:</span> {departmentDetail.departmentEmail}</div>
                        <div><span className="font-medium">Contact:</span> {departmentDetail.contactNumber}</div>
                        <div><span className="font-medium">Active:</span> {departmentDetail.isActive ? 'Yes' : 'No'}</div>
                      </div>
                    ) : (
                      <div className="grid grid-cols-2 gap-2 mt-2">
                        <div>
                          <Label>Name</Label>
                          <input className="w-full border rounded px-2 py-1" value={editDeptFields.name} onChange={e => setEditDeptFields(f => ({ ...f, name: e.target.value }))} />
                        </div>
                        <div>
                          <Label>Description</Label>
                          <input className="w-full border rounded px-2 py-1" value={editDeptFields.description} onChange={e => setEditDeptFields(f => ({ ...f, description: e.target.value }))} />
                        </div>
                        <div>
                          <Label>Email</Label>
                          <input className="w-full border rounded px-2 py-1" value={editDeptFields.departmentEmail} onChange={e => setEditDeptFields(f => ({ ...f, departmentEmail: e.target.value }))} />
                        </div>
                        <div>
                          <Label>Contact</Label>
                          <input className="w-full border rounded px-2 py-1" value={editDeptFields.contactNumber} onChange={e => setEditDeptFields(f => ({ ...f, contactNumber: e.target.value }))} />
                        </div>
                        <div className="flex items-center gap-2">
                          <input type="checkbox" checked={!!editDeptFields.isActive} onChange={e => setEditDeptFields(f => ({ ...f, isActive: e.target.checked }))} />
                          <span>Active</span>
                        </div>
                        <div className="flex gap-2 mt-2">
                          <Button size="sm" onClick={async () => {
                            ...
                            setDeptError(null)
                            try {
                              const res = await fetch(`/api/department/${departmentDetail.id}`, {
                                method: 'PUT',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify(editDeptFields),
                              })
                              if (!res.ok) throw new Error('Failed to update department')
                              const updated = await res.json()
                              setDepartmentDetail({ ...editDeptFields })
                          ...
                          <Button size="sm" variant="ghost" onClick={() => setEditingDepartment(false)}>Cancel</Button>
                        </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              {isSystemAdmin && <Button variant="secondary" onClick={handleSend}>Send report</Button>}
