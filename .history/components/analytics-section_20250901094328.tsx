  ...
  useEffect(() => {
    fetchStatusDropdown().then(setStatusOptions).catch(() => setStatusOptions([]));
  }, []);
"use client"
...
function AnalyticsSectionInner({ reports: externalReports }: AnalyticsSectionProps) {
  ...
  const statusNameMap = statusOptions.reduce<Record<string, string>>((acc, s) => {
    acc[s.id] = s.name;
  ...
  <div>
      <div className="grid gap-6">
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
          {(() => {
            ...
            return ["systemadmin", "sysadmin"].includes(normalizedRole);
          })() && (
      ...
      <div className="grid grid-cols-1 gap-4">
        {/* Interactive charts for System Admin: by Status, Department, Severity */}
        {(() => {
          const normalizedRole = (user?.role || "").replace(/\s+/g, "").toLowerCase();
          if (["systemadmin", "sysadmin"].includes(normalizedRole)) {
            return (
              <>
                <Card>
                  <CardHeader>
                  ...
                  <CardContent>
                    <BarChart width={400} height={300} data={Object.entries(reportsByStatus).map(([status, count]) => ({ status: statusNameMap[status] || status, count }))}>
                ...
                <Card>
                  ...
                  <CardContent>
                    <BarChart width={400} height={300} data={Object.entries(reportsByDepartment).map(([department, count]) => ({ department, count }))}>
          ...
          // For Department Admin: only Status and Severity
            ...
            return (
              <>
                <Card>
                  ...
                  <CardContent>
                    <BarChart width={400} height={300} data={Object.entries(reportsByStatus).map(([status, count]) => ({ status, count }))}>
                      <XAxis dataKey="status" />
                      <YAxis />
                      <Tooltip />
                      <Legend />
              ...
              <Card>
                ...
                <CardContent>
                  {Object.keys(reportsByDepartment).length > 0 ? (
                    <BarChart width={400} height={200} data={Object.entries(reportsByDepartment).map(([department, count]) => ({ department, count }))}>
                      <XAxis dataKey="department" />
                      <YAxis />
                      <Tooltip />
                      <Legend />
        ...
        <Card>
          ...
          <CardContent>
            {Object.keys(reportsBySeverity).length > 0 ? (
              <BarChart width={400} height={200} data={Object.entries(reportsBySeverity).map(([severity, count]) => ({ severity, count }))}>
                <XAxis dataKey="severity" />
                <YAxis />
                <Tooltip />
                <Legend />
      ...
      <Card>
        ...
        <CardContent>
          {Object.keys(reportsByStatus).length > 0 ? (
                    <LineChart width={400} height={200} data={Object.entries(reportsByStatus).map(([status, count]) => ({ status: statusNameMap[status] || status, count }))}>
                      <XAxis dataKey="status" />
                      <YAxis />
                      <Tooltip />
                      <Legend />
