  ...
  const handleUpdateDepartment = async (deptId: string, updated: { name?: string; description?: string; departmentEmail?: string; contactNumber?: string; isActive?: boolean }) => {
    try {
      const body = {
        ...
        departmentEmail: updated.departmentEmail,
      ...
      setDepartments(prev => prev.map(d => d.id === deptId ? { ...d, name: obj.departmentName ?? obj.departmentName ?? updated.name ?? d.name, description: obj.description ?? updated.description ?? d.description, departmentEmail: obj.departmentEmail ?? updated.departmentEmail ?? d.departmentEmail, contactNumber: obj.contactNumber ?? updated.contactNumber ?? d.contactNumber, isActive: obj.isActive ?? (updated.isActive ?? d.isActive) } : d))
    } catch (e) {
                  ...
                  <div className="bg-white rounded-lg shadow p-6 border border-gray-200">
                    ...
                    <ul className="space-y-4">
                      {departments.map((dept) => (
                        <li key={dept.id} className="bg-gray-50 rounded px-4 py-3">
                          ...
                          <div className="flex items-center justify-between mt-3">
                            ...
                            <div className="text-right text-xs text-muted-foreground">
                              <div>Created: {dept.createdAt ? new Date(dept.createdAt).toLocaleString() : '-'}</div>
                              <div>Updated: {dept.updatedAt ? new Date(dept.updatedAt).toLocaleString() : '-'}</div>
                            </div>
                          </div>
                        </li>
                      ))}
