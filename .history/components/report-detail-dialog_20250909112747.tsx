...
import { fetchReportDetail, postReportAction } from '@/lib/reports'
...
export default function ReportDetailDialog({ report, isOpen, onClose }: ReportDetailDialogProps) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // append internal comment locally
      if (newComment.trim()) {
        setComments(prev => [
          ...prev,
          { author: user?.fullName || 'You', timestamp: new Date().toISOString(), text: newComment.trim() }
        ]);
        setNewComment("");
      }
      setDetailsError(null);
      setSaveMessage('Saved'); // keep dialog open so user can choose Send
    } catch (e: any) {
  ...
  async function handleSend() {
    ...
    setSaveMessage(null);
    try {
      // ensure latest changes + comment saved first
      await postReportAction(report.id, 'save', {
        department: currentDepartment,
        severity: currentSeverity,
        status: currentStatus,
  ...
  // default fallback options for severity & departments if dropdown not available
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              {isSystemAdmin && <Button variant="secondary" onClick={handleSend} disabled={submitting}>{submitting ? 'Sending...' : 'Send report'}</Button>}
