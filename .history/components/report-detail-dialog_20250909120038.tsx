...
export default function ReportDetailDialog({ report, isOpen, onClose }: ReportDetailDialogProps) {
  ...
  async function handleSend() {
    ...
    setSubmitting(true);
    try {
      // include currentDepartment so backend routes to respective department
          ...
          <div className="grid gap-4">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              {(isSystemAdmin || isDepartmentAdmin) && (
                <Button variant="secondary" onClick={handleSend} disabled={submitting || (dirty && !hasSaved)}>
                  {submitting ? 'Working...' : 'Send report'}
                </Button>
              )}
