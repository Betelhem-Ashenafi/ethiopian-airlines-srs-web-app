...
import { logEvent } from "@/lib/logClient"
...
export default function SettingsSection() {
  ...
  const handleAddDepartment = async () => {
    ...
    try {
      ...
      logEvent({
        ...
        message: `Department added: ${item?.name || newDepartmentName}`,
        data: item,
        user
      })
    } catch (e: any) {
      ...
      logEvent({
        type: 'error',
        action: 'add_department',
        message: `Failed to add department: ${newDepartmentName}`,
  ...
  const handleDeleteDepartment = async (deptId: string) => {
    ...
    if (!serverId) {
      ...
      logEvent({
        ...
        message: `Locally deleted department: ${dept?.name || deptId}`,
        data: dept,
        user
      })
      return
    ...
    try {
      ...
      logEvent({
        type: 'info',
        action: 'delete_department',
        message: `Deleted department: ${dept?.name || deptId}`,
        data: dept,
        user
      })
    } catch (e: any) {
      ...
      logEvent({
        type: 'error',
        action: 'delete_department',
        message: `Failed to delete department: ${dept?.name || deptId}`,
        data: { error: e?.message },
        user
      })
    ...
    try {
      ...
      if (isNaN(Number(editDeptId))) {
        ...
        logEvent({
          ...
          message: `Created department via update modal: ${merged.DepartmentName}`,
          data: payload,
          user
        })
        closeEditDept()
      ...
      if (!putId) {
        ...
        logEvent({
          type: 'info',
          action: 'update_department',
          message: `Created department via update modal (fallback): ${merged.DepartmentName}`,
          data: payloadC,
          user
        })
        closeEditDept()
      ...
      if (!res.ok) {
        ...
        if (res.status === 404) {
          ...
          logEvent({
            type: 'info',
            action: 'update_department',
            message: `Created department via update modal (PUT fallback): ${merged.DepartmentName}`,
            data: payloadC,
            user
          })
          closeEditDept()
      ...
      logEvent({
        type: 'info',
        action: 'update_department',
        message: `Updated department: ${merged.DepartmentName}`,
        data: payload,
        user
      })
      closeEditDept()
      ...
      logEvent({
        type: 'error',
        action: 'update_department',
        message: `Failed to update department: ${editDeptFields.name}`,
        data: { error: e?.message },
        user
      })
  ...
  const handleAddLocation = () => {
    if (newLocation && !locations.some(l => l.locationName === newLocation)) {
      const newLoc = { locationID: Date.now(), locationName: newLocation, isActive: true }
      setLocations([...locations, newLoc])
      setNewLocation("")
      logEvent({
        type: 'info',
        action: 'add_location',
        message: `Location added: ${newLocation}`,
        data: newLoc,
        user
      })
    }
  ...
  const handleDeleteLocation = (locId: number) => {
    ...
    logEvent({
      ...
      message: `Location deleted: ${loc?.locationName || locId}`,
      data: loc,
      user
    })
  }
  const handleUpdateLocation = (locId: number, updated: string) => {
    ...
    logEvent({
      ...
      message: `Location updated: ${loc?.locationName || locId} -> ${updated}`,
      data: { before: loc, after: { ...loc, locationName: updated } },
      user
    })
  }
          ...
          <Card>
            ...
            <CardContent className="grid gap-4">
              <div className="grid gap-2">
                ...
                <Input id="fullName" defaultValue={user?.fullName || ""} disabled className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="employeeID">Employee ID</Label>
                <Input id="employeeID" defaultValue={user?.employeeID || ""} disabled className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="email">Email</Label>
                <Input id="email" defaultValue={user?.email || ""} disabled className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="role">Role</Label>
                <Input id="role" defaultValue={user?.role || ""} disabled className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
              </div>
              {user?.role === "Department Admin" && user.department && (
                <div className="grid gap-2">
                  <Label htmlFor="department">Department</Label>
                  <Input id="department" defaultValue={user.department} disabled className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
                </div>
              )}
              <Separator />
              <div className="grid gap-2">
          ...
          <TabsContent value="department">
            <Card>
              ...
              <CardContent className="grid gap-8">
                  <div className="bg-neutral-50 dark:bg-neutral-800 rounded-lg shadow p-6 border border-gray-200 dark:border-neutral-700">
                    ...
                    <div className="overflow-x-auto">
                      <div className="flex flex-col gap-2">
                          {departments.map((dept) => (
                            <div key={dept.id} className="bg-neutral-50 dark:bg-neutral-800 rounded-md p-4 border border-gray-100 dark:border-neutral-700 flex items-center justify-between">
                          ...
                          <Dialog open={!!editDeptId} onOpenChange={closeEditDept}>
                            <DialogContent className="max-w-md">
                              ...
                              <div className="grid gap-3">
                                <div>
                                  <Label>Name</Label>
                                  <Input value={editDeptFields.name} disabled={!isEditingDept} onChange={e => setEditDeptFields(f => ({ ...f, name: e.target.value }))} className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
                                </div>
                                <div>
                                  <Label>Description</Label>
                                  <Input value={editDeptFields.description} disabled={!isEditingDept} onChange={e => setEditDeptFields(f => ({ ...f, description: e.target.value }))} className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
                                </div>
                                <div>
                                  <Label>Email</Label>
                                  <Input value={editDeptFields.departmentEmail} disabled={!isEditingDept} onChange={e => setEditDeptFields(f => ({ ...f, departmentEmail: e.target.value }))} className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
                                </div>
                                <div>
                                  <Label>Contact</Label>
                                  <Input value={editDeptFields.contactNumber} disabled={!isEditingDept} onChange={e => setEditDeptFields(f => ({ ...f, contactNumber: e.target.value }))} className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
                                </div>
                                <div className="flex items-center gap-2">
              ...
              <CardContent className="grid gap-8">
                <div className="bg-neutral-50 dark:bg-neutral-800 rounded-lg shadow p-6 border border-gray-200 dark:border-neutral-700">
                  ...
                  <div className="overflow-x-auto">
                    <div className="flex flex-col gap-2">
                      {locations.map((loc) => (
                        <div key={loc.locationID} className="bg-neutral-50 dark:bg-neutral-800 rounded-md p-4 border border-gray-100 dark:border-neutral-700 flex items-center justify-between">
                          <div>
          ...
          <DialogContent className="max-w-md">
            ...
            {editLocFields && (
              <div className="grid gap-3">
                <div>
                  <Label>Name</Label>
                  <Input value={editLocFields.locationName} disabled className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
                </div>
                <div>
                  <Label>ID</Label>
                  <Input value={String(editLocFields.locationID)} disabled className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
                </div>
                <div>
                  <Label>Active</Label>
                  <Input value={editLocFields.isActive ? 'Yes' : 'No'} disabled className="bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-black dark:text-white" />
                </div>
                <div className="flex gap-2 justify-end mt-2">
