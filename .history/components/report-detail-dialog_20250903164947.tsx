...
export default function ReportDetailDialog({ report, isOpen, onClose }: ReportDetailDialogProps) {
  ...
  // compute image source via proxy when available; normalize leading slashes to avoid '//' issues
  let rawImageUrl = fetched?.imageUrl ?? report.imageUrl
  if (rawImageUrl && rawImageUrl.startsWith('/') && rawImageUrl.startsWith('//') === false) {
    // keep a single leading slash if provided; strip duplicate slashes when concatenating elsewhere
    rawImageUrl = rawImageUrl.replace(/^\/+/, '/');
  }
  if (rawImageUrl && rawImageUrl.startsWith('//')) {
    // strip protocol-relative double-slash
    rawImageUrl = rawImageUrl.replace(/^\/\//, '/');
  }
