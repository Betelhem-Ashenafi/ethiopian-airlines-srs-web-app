...
function AnalyticsSection({ reports: externalReports }: AnalyticsSectionProps) {
  ...
  // If department admin receives only single-dept data, attempt to fetch all
  useEffect(() => {
    async function maybeFetchAll() {
      if (!user) return
      const isDeptAdmin = user.role === 'Department Admin'
      if (!isDeptAdmin) return
      const uniqueDepts = new Set((allReports || []).map(r => r.departmentName || r.aiDepartment).filter(Boolean))
      if (uniqueDepts.size <= 1) {
        try {
          ...
          setLoadError(e?.message || 'Failed to load full reports list')
        } finally {
          setLoading(false)
        }
      }
  ...
  const scopedReports = departmentScope === 'all' ? reports : reports.filter(r => (r.departmentName || r.aiDepartment) === departmentScope)
    ...
    <div>
      <div className="grid gap-6">
        ...
        <div className="flex flex-wrap gap-4 items-center">
          ...
          {(() => {
            const unique = Array.from(new Set(reports.map(r => (r.departmentName || r.aiDepartment)).filter(Boolean)))
            if (unique.length > 1) {
              return (
                <Select value={departmentScope} onValueChange={setDepartmentScope}>
                  <SelectTrigger className="w-[200px]"><SelectValue placeholder="Dept Scope" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">All Departments</SelectItem>
                    {unique.map(d => <SelectItem key={d} value={d}>{d}</SelectItem>)}
                  </SelectContent>
                </Select>
              )
            }
