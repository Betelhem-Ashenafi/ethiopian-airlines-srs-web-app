...
export default function ReportDetailDialog({ report, isOpen, onClose }: ReportDetailDialogProps) {
  ...
  const departmentOptions = ["Facility Maintenance", "IT Support", "Plumbing", "Safety", "Other"];
    // Persist changes to backend via rewrite (/api/reports/* -> backend)
    try {
      const payload: any = {
        reportID: report.id,
        // include both id and name when available
      ...
      // update local fetched state from response if backend returns data
  ...
  const handleSend = async () => {
    // Only system admin should be able to send
    ...
    try {
      ...
      const json = await res.json()
      console.log('Sent successfully', json)
      // optionally mark sync status or update UI
