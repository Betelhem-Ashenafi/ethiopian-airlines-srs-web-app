        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button className="inline-flex items-center gap-2 rounded-full bg-et-gold text-et-green hover:bg-yellow-400 px-3 py-1">
              <Avatar>
                <AvatarImage src={user && (user as any).avatar ? (user as any).avatar : "/placeholder-user.jpg"} alt="User Avatar" />
                <AvatarFallback>
                  {user && user.fullName ? user.fullName.charAt(0).toUpperCase() : "?"}
                </AvatarFallback>
              </Avatar>
              {/* Show name next to avatar on medium+ screens */}
            ...
            <div className="p-3">
              <div className="flex items-center gap-3">
                <Avatar className="h-12 w-12">
                  <AvatarImage src={user && (user as any).avatar ? (user as any).avatar : "/placeholder-user.jpg"} alt="User Avatar" />
                  <AvatarFallback>{user && user.fullName ? user.fullName.charAt(0).toUpperCase() : "?"}</AvatarFallback>
                </Avatar>
