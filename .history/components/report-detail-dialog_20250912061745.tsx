...
export default function ReportDetailDialog({ report, isOpen, onClose, onUpdate }: ReportDetailDialogProps) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (newComment.trim()) {
        try {
          await addReportComment(report.id, newComment.trim(), user?.fullName || 'You')
          setNewComment("")
          // Reload full list to include any legacy or concurrent comments
          const updated = await fetchReportComments(report.id)
          setComments(updated.comments || [])
        } catch (e) {
          // fallback optimistic add then attempt refresh silently
          setComments(prev => [...prev, { author: user?.fullName || 'You', timestamp: new Date().toISOString(), text: newComment.trim() }])
          setNewComment("")
          fetchReportComments(report.id).then(d => setComments(d.comments || [])).catch(()=>{})
        }
      }
