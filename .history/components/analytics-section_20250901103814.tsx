...
function AnalyticsSectionInner({ reports: externalReports }: AnalyticsSectionProps) {
  // Department options state for dropdowns
  const [departmentOptions, setDepartmentOptions] = useState<{ id: string; name: string }[]>([]);
  useEffect(() => {
    fetchDepartmentsDropdown().then(setDepartmentOptions);
  }, []);
        ...
        <Card>
          ...
          <CardContent>
            {statusChartData.length > 0 ? (
              renderChart(chartType, statusChartData, "count", "status", etGreen)
            ) : (
        ...
        <Card>
          ...
          <CardContent>
            {departmentChartData.length > 0 ? (
              renderChart(chartType, departmentChartData, "count", "department", etGold)
            ) : (
              <div className="text-center text-muted-foreground">No data for this filter.</div>
            )}
          </CardContent>
